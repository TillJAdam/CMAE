...
run_models <- function(data, label_time) {
  map_dfr(outcomes, function(var) {
    formula <- as.formula(paste0(var, " ~ 1 + Time + Sex + Race + Med_Status + Diagnosis_Class + (1 + Time | ID)"))
...
results <- map_dfr(names(groups), function(group) {
  ...
  map_dfr(outcomes, function(outcome) {
    model_all <- lmer(as.formula(paste0(outcome, " ~ Time + Sex + Race + Med_Status + Diagnosis_Class + (1 + Time | ID)")), 
                      data = data_all, REML = FALSE)
    model_03  <- lmer(as.formula(paste0(outcome, " ~ Time + Sex + Race + Med_Status + Diagnosis_Class + (1 + Time | ID)")), 
...
results <- purrr::map_dfr(names(groups), function(group_name) {
  ...
  purrr::map_dfr(outcomes, function(outcome) {
    f_weight   <- as.formula(paste0(outcome, " ~ Time + Weight + Sex + Race + Med_Status + Diagnosis_Class + (1 + Time | ID)"))
    f_fatmass  <- as.formula(paste0(outcome, " ~ Time + Fat_Mass + Sex + Race + Med_Status + Diagnosis_Class + (1 + Time | ID)"))
